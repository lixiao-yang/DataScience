...
(define/contract (nnf? E)
  ...
  (cond
    ...
    [(or (is-AND? E) (is-OR? E)) (and (nnf? (second E)) (nnf? (third E)))]
    [else #f])
...
(define/contract (makeNNF E)
  ...
  (cond
    [(nnf? E) E]
    [(is-AND? E) (list 'AND (makeNNF (second E)) (makeNNF (third E)))]
    [(is-OR? E) (list 'OR (makeNNF (second E)) (makeNNF (third E)))]
    [(is-IMPLIES? E) (list 'OR (makeNNF (list 'NOT (second E))) (makeNNF (third E)))]
    [(is-EQUIV? E) (list 'AND (list 'OR (makeNNF (list 'NOT (second E))) (makeNNF (third E))) (list 'OR (makeNNF (list 'NOT (third E))) (makeNNF (second E))))]
    [(is-XOR? E) (list 'AND (list 'OR (makeNNF (second E)) (makeNNF (third E))) (list 'OR (makeNNF (list 'NOT (second E))) (makeNNF (list 'NOT (third E)))))]
    ;Situations for NOT
    [(is-constant? (second E)) (if (equal? (second E) 'TRUE) 'FALSE 'TRUE)]
    [(is-NOT? (second E)) (makeNNF (second (second E)))]
    [(is-AND? (second E)) (list 'OR (makeNNF (list 'NOT (second (second E)))) (makeNNF (list 'NOT (third (second E)))))]
    [(is-OR? (second E)) (list 'AND (makeNNF (list 'NOT (second (second E)))) (makeNNF (list 'NOT (third (second E)))))]
    [(is-EQUIV? (second E)) (makeNNF (list 'XOR (second (second E)) (third (second E))))]
    [(is-XOR? (second E)) (makeNNF (list 'EQUIV (second (second E)) (third (second E))))]
    [(is-IMPLIES? (second E)) (list 'AND (makeNNF (second (second E))) (makeNNF (list 'NOT (makeNNF (third (second E))))))]
    )) ; delete this False line and put your implementation here on these lines. do NOT delete the ;end comment after your definition
...
(define/contract (addTerm T P) (-> term? polyList? polyList?)
  (cond
    [(null? P) (list T)] 
    [(> (second T) (second (first P))) (cons T P)]
    [(= (second T) (second (first P)))
     (if (zero? (+ (first T) (first (first P))))
         (rest P)
         (cons (list (+ (first T) (first (first P))) (second T)) (rest P)))]
    [else (cons (first P) (addTerm T (rest P)))])
...
(define/contract (multTerm T P) (-> term? polyList? polyList?)
  (if (null? P)
      '()
      (cons 
       (list (* (first T) (first (first P))) (+ (second T) (second (first P))))
       (multTerm T (rest P))))
  ) ; delete the null and put your implementation here on these lines. do NOT delete the ;end comment after your definition
...
(define (multiply-term term poly) 
  (map (lambda (p) (list (* (first term) (first p)) (+ (second term) (second p)))) poly))

(define (multiply-terms P Q)
  (foldr append '() (map (lambda (p) (multiply-term p Q)) P)))

(define/contract (multPolys P Q) (-> polyList? polyList? polyList?)
  (foldr addTerm '() (multiply-terms P Q))
  ); delete the null and put your implementation here on these lines. do NOT delete the ;end comment after your definition
